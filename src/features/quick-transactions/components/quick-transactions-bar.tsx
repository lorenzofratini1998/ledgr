    ...
    <>
      <div className="w-full">
        <Accordion className="w-full">
          <AccordionItem
            value="quick-transactions"
            className="rounded-2xl border border-border/75 bg-card/50 hover:bg-card/80 transition-all shadow-2xs hover:border-border overflow-hidden"
          >
            <div className="flex items-center justify-between gap-3 px-4 py-2 sm:px-5 sm:py-2.5">
              <AccordionTrigger className="hover:no-underline py-1 text-foreground font-semibold flex-1 gap-3 cursor-pointer items-center">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-7 h-7 rounded-xl bg-amber-500/10 text-amber-500 dark:bg-amber-500/20 shrink-0">
                    <Zap className="w-4 h-4 fill-amber-500/40" />
                  </div>
                  <span className="text-sm font-semibold tracking-tight text-foreground">
                    {t('quickTransactions.title')}
                  </span>
                  <Badge variant="secondary" className="text-[10px] font-medium h-5 px-2 rounded-md text-muted-foreground">
              ...
              <Button
                ...
                className="h-8 px-2.5 text-xs text-muted-foreground hover:text-foreground gap-1.5 rounded-lg shrink-0"
              >
            ...
            <AccordionContent className="px-4 pb-4 sm:px-5 sm:pb-5 pt-1">
              {/* Empty State Banner (if no quick transactions exist) */}
              {quickTransactions.length === 0 ? (
                <div className="flex flex-col sm:flex-row items-center justify-between p-4 rounded-2xl border border-dashed border-border/80 bg-card/40 hover:bg-card/70 transition-all gap-3 mt-1">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 pt-1">
                  {quickTransactions.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative flex flex-col justify-between p-3.5 rounded-2xl border text-left bg-card hover:bg-accent/40 active:scale-[0.98] transition-all duration-200 shadow-2xs hover:shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring min-h-[100px] ${
                  ...
                  {quickTransactions.length < 5 && (
                    <button
                      type="button"
                      onClick={() => setIsDialogOpen(true)}
                      className="flex flex-col items-center justify-center p-3.5 rounded-2xl border border-dashed border-border/80 hover:border-primary/50 hover:bg-primary/5 text-muted-foreground hover:text-foreground active:scale-[0.98] transition-all duration-200 shadow-2xs min-h-[100px] gap-2"
                    >
