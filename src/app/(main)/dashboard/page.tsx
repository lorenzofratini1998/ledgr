...
export default async function DashboardPage({
  ...
  const { from, to, resolvedPeriod } = await parsePeriod(period, resolvedParams?.from, resolvedParams?.to);
    ...
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col gap-6">
        ...
        <Suspense fallback={<BalanceTrendWidgetSkeleton className="col-span-full" />}>
          <BalanceTrendWidget from={from} to={to} className="col-span-full" />
        </Suspense>

        {/* Category Breakdown (Expenses) */}
        <Suspense fallback={<CategoryBreakdownWidgetSkeleton className="col-span-full" />}>
          <CategoryBreakdownWidget from={from} to={to} className="col-span-full" />
        </Suspense>
      </div>
        ...
        <div className="grid gap-6 grid-cols-1 xl:grid-cols-3">
          <Suspense fallback={<IncomeVsExpensesWidgetSkeleton className="xl:col-span-2" />} key={`income-${from}-${to}`}>
             <IncomeVsExpensesWidget from={from} to={to} className="xl:col-span-2" />
          </Suspense>
